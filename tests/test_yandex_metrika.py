...
def test_default_search_segment_uses_last_significant_attribution(identity, yandex_settings):
    ...
    assert any(call.get("dimensions") == "ym:s:<attribution>TrafficSource" for call in api.calls)
    assert any(call.get("dimensions") == "ym:s:<attribution>SearchEngineRoot" for call in api.calls)
    assert any(
